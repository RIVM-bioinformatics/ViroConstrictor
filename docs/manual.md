...
| `--match-ref` /<br>`-mr` | N/A | Enables the match-ref process for all samples in the analysis. Please see the additional information regarding the [match reference process](multi-reference.md#choose-the-best-reference-for-each-sample) |
| `--segmented` /<br>`-seg` | N/A | Indicates that samples to be analyzed are segmented instead of a single reference genome. <br> Setting is only applicable to the match-ref process. Please see the addtional information regarding the [match reference process for segmented viruses](multi-reference.md#choose-the-best-reference-for-each-sample-with-segmented-viruses) |
