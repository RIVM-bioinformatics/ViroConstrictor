...
class Scheduler(Enum):
    ...
    def _scheduler_from_argument(
        ...
        if scheduler_str:
            if not cls.is_valid(scheduler_str):
                log.warning(f"Invalid scheduler string: '{scheduler_str}', using non-grid mode")
                return cls.LOCAL
            scheduler = cls.from_string(scheduler_str)
            if scheduler == cls.AUTO:
                log.debug("Helper functionality :: Scheduler :: Scheduler set to AUTO, trying to determine automatically")
                return None
            log.debug(f"Helper functionality :: Scheduler :: Scheduler determined from string: '{scheduler_str}'")
            return scheduler
        return None
    ...
    def _scheduler_from_environment(cls) -> Optional["Scheduler"]:
        if shutil.which("sbatch") or "SLURM_JOB_ID" in os.environ:
            log.debug("Helper functionality :: Scheduler :: Scheduler found in environment: SLURM")
            return cls.SLURM
        if shutil.which("bsub") or "LSB_JOBID" in os.environ:
            ...
            return cls.LSF
        log.debug("Helper functionality :: Scheduler :: No scheduler found in environment variables or executables.")
        return None
    ...
    def _scheduler_from_drmaa(cls) -> Optional["Scheduler"]:
        try:
            ...
            with drmaa.Session() as session:
                ...
                log.debug(f"Helper functionality :: Scheduler :: Scheduler determined from DRMAA: '{scheduler_name}'")
    ...
    def determine_scheduler(
        ...
        if scheduler_str:
            ...
            if scheduler is not None:
                log.debug(f"Helper functionality :: Scheduler :: Scheduler selected from argument: '{scheduler.name}'")
                return scheduler

        if user_config:
            scheduler = cls._scheduler_from_config(user_config)
            if scheduler is not None:
                log.debug(f"Helper functionality :: Scheduler :: Scheduler selected from config: '{scheduler.name}'")
                return scheduler

        scheduler = cls._scheduler_from_environment()
        if scheduler is not None:
            log.debug(f"Helper functionality :: Scheduler :: Scheduler selected from environment: '{scheduler.name}'")
            return scheduler

        scheduler = cls._scheduler_from_drmaa()
        if scheduler is not None:
            log.debug(f"Helper functionality :: Scheduler :: Scheduler selected from DRMAA: '{scheduler.name}'")
            return scheduler
