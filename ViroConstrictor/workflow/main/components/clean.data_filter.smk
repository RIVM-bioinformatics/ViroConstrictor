    ...
    rule qc_filter:
        ...
        params:
            adapter_removal_settings=lambda wc: get_preset_parameter(
                ...
                parameter_name=f"Fastplong_AdapterRemoval_Settings_{config['platform']}",
            ),
            per_read_qc_settings=lambda wc: get_preset_parameter(
                preset_name=SAMPLES[wc.sample]["PRESET"],
                parameter_name=f"Fastplong_PerReadQC_Settings_{config['platform']}",
            ),
            slidingwindow_qc_settings=lambda wc: get_preset_parameter(
                preset_name=SAMPLES[wc.sample]["PRESET"],
                parameter_name=f"Fastplong_QC_SlidingWindow_Settings_{config['platform']}",
            ),
            lowcomplexity_filter_settings=lambda wc: get_preset_parameter(
                preset_name=SAMPLES[wc.sample]["PRESET"],
                parameter_name=f"Fastplong_LowComplexityFilter_Settings_{config['platform']}",
            ),
            minlength_filter_settings=lambda wc: get_preset_parameter(
                preset_name=SAMPLES[wc.sample]["PRESET"],
                parameter_name=f"Fastplong_MinReadLength_Settings_{config['platform']}",
            ),
        shell:
            """
            fastplong --thread {threads} -i {input} \
