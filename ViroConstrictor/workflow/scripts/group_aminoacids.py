...
class GroupAminoAcids(BaseScript):
    ...
    def _group_amino_acids(self) -> None:
        ...
        def write_sequences(virus: str, ref_id: str, feature_name: str, output_files: list[str], seq_records_df: pd.DataFrame) -> None:
        ...
        for virus in unique_viruses:
            unique_ref_ids = np.array(seq_records_df.loc[seq_records_df["Virus"] == virus]["RefID"].unique(), dtype=np.str_)
